...
/**
 * V4.5 — V4 with a custom IPlayer (WynnSolverPlayer). Algorithm core
 * (SWAR, BFS, three-phase) is unchanged. Only the IPlayer access shifts:
 *
 *  - Base SP read directly from player.allocated[] (5x int loads) instead
 *    of 5x player.allocated(SkillPoint) — each call costs an interface
 *    dispatch + ordinal() lookup + array access.
 *
 *  - finishResult writes the aggregated bonus vector once via direct
 *    field access on the concrete player, replacing per-kept-item
 *    player.modify(bonuses, true) calls (each with its own 5-iteration
 *    loop and weight update).
 *
 * SWAR/BFS authorship (kcinsoft/AiverAiva) is the same as V4 — see V4's
 * javadoc. V4.5's contribution is purely the IPlayer integration.
 */
@Information(name = "WynnSolver", version = 45, authors = {"Alex-Guha", "kcinsoft", "AiverAiva"})
public class WynnSolverV45Algorithm implements IAlgorithm<WynnSolverPlayer> {
    ...
    public Result run(WynnSolverPlayer player) {
        ...
        // ── Phase 2: scalar greedy fixed-point ────────────────────────
        int activeMask = 0;
    ...
    private static Result finishResult(IEquipment[] items, int[][] bonusesRaw, boolean[] keep,
